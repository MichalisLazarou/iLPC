...
# ---- data loading
    ...
    #time1 = time.time()
    # for i in range(100):
    #     #ndatas =torch.unsqueeze(ndatas[0], 0)
    ...
    # time2 = time.time()
    # time_ici = time2-time1

    time1 = time.time()
    acc_mine, acc_std = trans_ilpc(params, ndatas, labels, n_lsamples)
    time2 = time.time()
    time_ilpc = time2 - time1
    #print('DATASET: {}, final accuracy ici: {:0.2f} +- {:0.2f}, shots: {}, queries: {}, time: {}'.format(dataset, acc_ici * 100, acc_std_ici * 100, n_shot, n_queries, time_ici/100))
    #print("final accuracy PT-MAP: {:0.2f} +- {:0.2f}, time: {}".format(*(100 * x for x in acc_test), time_pt/100))
    print('DATASET: {}, final accuracy ilpc: {:0.2f} +- {:0.2f}, shots: {}, queries: {}, time: {}'.format(dataset, acc_mine * 100,acc_std * 100, n_shot, n_queries, time_ilpc/100))
