...
def semi_ici(opt, X, Y, q, q_ys, labelled_samples):
    ...
    # for i in range(100):
        ...
        if opt.truncate=='yes':
            support_features, support_ys, _ = truncate(support_features, support_ys, support_ys)
...
def semi_ilpc(opt, X, Y, q, q_ys, labelled_samples):
    ...
    for i in range(X.shape[0]):
        ...
        support_ys, support_features, w = igf.iter_balanced_distractive(opt, support_features, support_ys, unlabelled_features, unlabelled_ys, labelled_samples)
        if opt.truncate== 'yes':
...
def distractor_features_pt(opt, ndatas, labels, labelled_samples, query_features, query_ys):
    ...
    # print(support_ys.shape, unlabelled_ys.shape)
    mask_support = ~support_ys.ge(opt.n_ways)
    # print(mask_support.shape, support_features.shape)
    ...
    return ndatas, labels, params.n_ways*params.n_shots, query_features, query_ys


def semi_pt(opt, xs, ys, xq, yq):
    acc =[]
    # for i in range(100):
...
if __name__ == '__main__':
    ...
    if params.algorithm == 'ptmap':
        ...
        acc_pt, acc_std_pt = semi_pt(params, ndatas, olabels, query_data, q_labels)
        print('DATASET: {}, final accuracy semi_pt: {:0.2f} +- {:0.2f}, shots: {}, unlabelled: {}, truncation: {}'.format(dataset,acc_pt * 100, acc_std_pt * 100, n_shot, n_unlabelled, params.truncate))
    if params.algorithm == 'ici':
        acc_ici, acc_std_ici = semi_ici(params, ndatas, labels, query_data, q_labels, n_lsamples)
        print('DATASET: {}, final accuracy ici: {:0.2f} +- {:0.2f}, shots: {}, unlabelled: {}, truncation: {}'.format(dataset, acc_ici * 100, acc_std_ici * 100, n_shot, n_unlabelled, params.truncate))
    if params.algorithm == 'ilpc':
        acc_mine, acc_std = semi_ilpc(params, ndatas, labels, query_data, q_labels, n_lsamples)
        print('DATASET: {}, final accuracy ilpc: {:0.2f} +- {:0.2f}, shots: {}, unlabelled: {}, truncation: {}, sinkhorn: {}, n_distractors: {}, distractive version: {}'.format(dataset, acc_mine * 100, acc_std * 100, n_shot, n_unlabelled, params.truncate, params.sinkhorn_bal, params.n_distractors, params.distractive_version))
    if params.algorithm == 'plcm':
        ndatas = ndatas.cuda()
        labels = labels.cuda()
        acc_plcm, acc_std_plcm = semi_plcm(params, ndatas, labels, query_data, q_labels, n_lsamples)
        print('DATASET: {}, final accuracy plcm: {:0.2f} +- {:0.2f}, shots: {}, unlabelled: {}, truncation: {}'.format(dataset, acc_plcm * 100, acc_std_plcm * 100, n_shot, n_unlabelled, params.truncate))
